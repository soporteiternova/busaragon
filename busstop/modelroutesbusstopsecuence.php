...
class modelroutesbusstopsecuence extends \BUSaragon\common\model {
    public $_database_collection = 'routesbusstopsecuence';
    public $code = '';
    public $line_id = '';
    public $name = '';
    public $number = '';
    ...
    public function update_from_api( $api_object, $api_endpoint ) {
        ...
        if ( $api_endpoint === \BUSaragon\common\controller::ENDOPOINT_BUS_ROUTES_BUSSTOP_SECUENCE_CTAZ && isset( $api_object->code ) ) {
            $array_criteria[] = [ 'code', 'eq', $api_object->code, 'string' ];
            ...
            if ( !empty( $array_obj ) ) {
                ...
                $this->code = $api_object->code;
            }

            $this->line_id = $api_object->line_id;
            $this->name = $api_object->name;
            $this->number = $api_object->number;
            $this->route = $api_object->route;
            $this->network = $api_endpoint;
            $this->lat_lng = [ (float) $api_object->latitud, (float) $api_object->longitud ];
    ...
    public function object_encode_data( $to_utf8 = false ) {
        ...
        $array_string = [ 'code', 'line_id', 'name', 'number', 'route' ];
